...
class lobbyUser(Frame):
    def __init__(self, window, parent : tk.Canvas, images : list, nom : str, nb,nb_player : int, height : int = 420, width : int = 317, droiteg : str = "haut", hb : str = "droite",edit = False):
        ...
        self.text = self.parent.create_text((width-220)/2,3,fill="white",font=('Lilita One', config.Config.taillePolice[0]),text=self.nomJoueur,anchor=tk.CENTER)
        if self.edit:
        ...
        self.showIAButtons(False,False)
        ...
        if self.activeclavier == True:
            if len(self.touche) == 1:
                ...
                if len(self.nomJoueur) > 0:
                    self.nomJoueur = self.nomJoueur[:-1]
            ...
            if width > 300:
                ...
                self.parent.itemconfigure(self.text, text=self.nomJoueur, font=('Lilita One', config.Config.taillePolice[0]))
                tailles = self.parent.bbox(self.text)
                width = tailles[2] - tailles[0]
                if width > 300:
                    self.parent.itemconfigure(self.text, text=self.nomJoueur, font=('Lilita One', config.Config.taillePolice[2]))
    ...
    def showIAButtons(self,isMe,isAdmin):
        if isAdmin and not isMe:
            self.parent.itemconfigure(self.Bouton_Robot,state=tk.NORMAL)
        else:
